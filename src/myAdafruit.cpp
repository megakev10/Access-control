 ...
 */
/* Ca c'est une macro. C'est comme une syntaxe constante qui sera recopiee par le preprocesseeur partout ou sera appellee cette macro. Bon de nos jours,
 cette methode est deconseillee et on conseille plutot une vrai fonction car en recopiant la syntaxe, le preprocesseur ne controle rien et en cas de bug, 
 l'erreur est signalee dans la macro sans specifier la fonction reellement en cause. {__VA_ARGS__} signifie nbre d'arguments variable.*/  
#define GET_CMD_PACKET(...)                                                    \
  ...
  if (packet.type != FINGERPRINT_ACKPACKET)                                    \
    return FINGERPRINT_PACKETRECIEVEERR;
/* Cette macro permet au MCU d'envoyer des instructions (writeStructuredPacket) et de recuperer des donnees
(getStructuredPacket) au capteur. "FINGERPRINT_COMMANDPACKET" permet justement au capteur de savoir qu'il s'agit d'un ordre et non de simples donnees. */
/*!
 * @brief Sends the command packet
 */
#define SEND_CMD_PACKET(...)                                                   \
  GET_CMD_PACKET(__VA_ARGS__);                                                 \
  return packet.data[0];// On recupere la reponse du capteur
...
Adafruit_Fingerprint::Adafruit_Fingerprint(SoftwareSerial *ss,
  ...
  hwSerial = NULL;//on utilise pas le UART mais plutot
  swSerial = ss;// le software Serial
  mySerial = swSerial;
}
#endif
...
*/
/**************************************************************************/
/* ca c'est le constructeur. Ici on a un pointeur vers la classe hardware serial. Cette specification est faite pour le UART directement.*/
Adafruit_Fingerprint::Adafruit_Fingerprint(HardwareSerial *hs,
  ...
  swSerial = NULL;
#endif
  hwSerial = hs; // Comme ca "hwSerial" pointe aussi vers "Hardware Serial"
  mySerial = hwSerial; // Et on passe tout au pointeur de "Stream"
}
...
*/
/**************************************************************************/
/* On surcharge le constructeur pour pouvoir utiliser "Stream". L'objectif est de pouvoir etablir la communication quelque soit le flux present. "Stream est la 
classe qui ggere tout les flux."*/
Adafruit_Fingerprint::Adafruit_Fingerprint(Stream *serial, uint32_t password) {

  thePassword = password;
  theAddress = 0xFFFFFFFF;

  hwSerial = NULL;// Comme ca pour une implementation sans UART, on est sur que le "if" en bas ne se lance pas. On considere simplement un flux quelconque.
#if defined(__AVR__) || defined(ESP8266) || defined(FREEDOM_E300_HIFIVE1)
  swSerial = NULL;// Meme chose ici
#endif
  mySerial = serial;
}
...
void Adafruit_Fingerprint::begin(uint32_t baudrate) {
  ...
  if (hwSerial)// Qui s'active si l'implementation est faite avec UART.
...
/*Voici la fonction de stockage.
>> "FINGERPRINT_STORE" est la commande qui dit au capteur d'enregistrer les donnees de sa RAM vers sa flash
>> "0x01" c'est en fait pour imposer au capteur d'utiliser le buffer1 de sa RAM pour creer le modele a stocker (en fait la RAM compte 02 buffers generalement "Charbuffer1" et "Charbuffer2")*/
/**************************************************************************/
/*!
