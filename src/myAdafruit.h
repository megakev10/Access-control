...
public:
  ...
  Adafruit_Fingerprint(SoftwareSerial *ss, uint32_t password = 0x0);/*On implemente uniquement le SoftwareSerial sur ces plateformes car c'est la qu'il est pertinent.
  Sur d'autre MCU comme ESP32, on a UART donc pas besoin de ca. */ 
#endif
  ...
  boolean verifyPassword(void);/*"boolean verifyPassword();" veut dire exactement la meme chose en C++. Cette ecriture vient du C ou un () veut simplement
  dire "parametres inconnus" plutot que "pas de parametres". Et ce n'est pas tres bien pour le compilateur qui trouvait une ambiguite si plus tard on faisait 
  par exemple "verifyPassword(valeur);"*/
  uint8_t getParameters(void);

  uint8_t getImage(void);// Pas besoin de parametres, elle utilise juste les donnees de sa classe.
